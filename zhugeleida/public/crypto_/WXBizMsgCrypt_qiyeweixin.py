...
class Prpcrypt(object):
    ...
    def decrypt(self,text,corpid):
        ...
        print('--- from_corpid ---->>',from_corpid,xml_content)
...
class WXBizMsgCrypt(object):
    ...
    def EncryptMsg(self, sReplyMsg, sNonce, timestamp = None):
        ...
        #sEncryptMsg: 加密后的可以直接回复用户的密文，包括 msg_signature, timestamp, nonce, encrypt的xml格式的字符串,
    ...
    def DecryptMsg(self, sPostData, sMsgSignature, sTimeStamp, sNonce):
        ...
        print('------ ret,encrypt,touser_name ------>>',ret,encrypt,touser_name)
        if ret != 0:
            return ret, None
        sha1 = SHA1() 
        ...
        pc = Prpcrypt(self.key)
        print('--- self.m_sCorpid | self.key ----->>',self.m_sCorpid,self.key)

        ret,xml_content = pc.decrypt(encrypt,self.m_sCorpid)
        print('------ ret,xml_content --------->>',ret,xml_content)
