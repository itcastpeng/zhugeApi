...
def deal_search_time(data,q):
    ...
    forward_num = models.zgld_accesslog.objects.select_related('user').filter(user__company_id=company_id,action=6).filter(q).count()  # 被转发的总数-不包括转发产品
    saved_total_num = models.zgld_accesslog.objects.select_related('user').filter(user__company_id=company_id, action=8).filter(q).count()  # 保存手机号
...
def deal_line_info(data):
    ...
    if index_type == 1:  # 客户总数
        ...
        follow_num = models.zgld_follow_info.objects.select_related('user_customer_flowup').filter(user_customer_flowup__user__company=company_id).filter(q1).count()

        return follow_num

    elif index_type == 3:  # 浏览总数
        browse_num = models.zgld_accesslog.objects.select_related('user').filter(user__company_id=company_id,action=1).filter(q1).count()  # 浏览名片的总数(包含着保存名片)
        return browse_num

    elif index_type == 4:  # 被转发总数
        forward_num = models.zgld_accesslog.objects.select_related('user').filter(user__company_id=company_id,action=6).filter(q1).count()  # 被转发的总数-不包括转发产品
        return forward_num

    elif index_type == 5:  # 被保存总数
        saved_total_num = models.zgld_accesslog.objects.select_related('user').filter(user__company_id=company_id, action=8).filter(q1).count()  # 保存手机号
        return saved_total_num

    elif index_type == 6:  # 被赞总数
        objs = models.zgld_userprofile.objects.filter(company_id=company_id).filter(q1).values('company_id').annotate(Sum('praise'))
        praise__sum = 0
        if objs:
            obj = objs[0]
            praise__sum = obj.get('praise__sum')
