...
def data_overview_statistics(request):
    ...
    for obj in company_objs:
        ...
        bossLeida_acount_data_and_line_info('acount_data', get_data_1)
        ...
        for user_obj in userprofile_objs:
            ...
            bossLeida_acount_data_and_line_info('acount_data', get_data_2)

        get_data_3 = {
            'company_id': company_id
        }
        bossLeida_acount_data_and_line_info('line_info', get_data_3)
        ...
        for user_obj in userprofile_objs:
            ...
            bossLeida_acount_data_and_line_info('line_info', get_data_4)

        get_data_5 = {
            'company_id': company_id
        }
        bossLeida_acount_data_and_line_info('sales_ranking_customer_num', get_data_5)


        get_data_6 = {
            'company_id': company_id
        }
        bossLeida_acount_data_and_line_info('hudong_pinlv_customer_num', get_data_6)
...
def bossLeida_acount_data_and_line_info(oper_type, data):
    ...
    if oper_type == "acount_data":
        ...
        data['start_time'] = ''
        data['stop_time'] = ''
        ret_data['count_data'] = deal_search_time(data)
        ...
        ret_data['yesterday_data'] = deal_search_time(data)

        start_time = (now_time - timedelta(days=7)).strftime("%Y-%m-%d")
        stop_time = now_time.strftime("%Y-%m-%d")
        data['start_time'] = start_time
        data['stop_time'] = stop_time
        ret_data['nearly_seven_days'] = deal_search_time(data)

        start_time = (now_time - timedelta(days=30)).strftime("%Y-%m-%d")
        stop_time = now_time.strftime("%Y-%m-%d")
        data['start_time'] = start_time
        data['stop_time'] = stop_time
        ret_data['nearly_thirty_days'] = deal_search_time(data)
