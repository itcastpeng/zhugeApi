...
``` python
{
    ...
    "data":{
        "ret_data":[
            {
                "id":1,
                "belong_user": 18643129824,     # 小程序所属的关联账户
                "company_id":1,         
                "auditid" : 403723892   # 审核编号，如果有就会返回，没有则为空。
                "company_name":"大麦公司11",
                "version_num":"v1.1",       #版本号
                "template_id":0,            #开发模板ID
                "status":"提交失败",         #上传状态 | 审核代码状态 | 上线状态
                "status_time":"2018-08-14 16:45",    # 发生状态的时间
                "experience_qrcode":"statics/zhugeleida/imgs/admin/qr_code/wxd306d71b02c5075e_20180814_135904_QRCode.jpg",   #小程序体验二维码
                "reason":"提交审 核代码报错: 85009 : already submit a version under auditing hint: [0F6H703469030]"    # 如果有报错会有报错的理由。
            }
        ],
        "data_count":1
    }
...
返回结果说明：
``` python
   status = 0       status_text = '代码上传成功'
   status = 1      status_text = '代码上传失败'
   status = 2      status_text = '审核通过'
   status = 3     status_text =  '审核未通过'
   status = 4     status_text =  '审核中'
   status = 5     status_text =  '提交审核报错'
   status = 6   status_text =  '上线成功'
   status = 7   status_text = '上线失败'
