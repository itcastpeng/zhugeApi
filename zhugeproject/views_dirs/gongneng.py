...
def gongneng(request):
    ...
    if request.method == "GET":
        ...
        if forms_obj.is_valid():
            ...
            # 获取所有数据
            objs = models.project_Function.objects.select_related('oper_user', 'item_name').filter(
                q).order_by(order)
            count = objs.count()
            print('objs- --  ->', objs)
            if length != 0:
    ...
    if request.method == "POST":
        ...
        if oper_type == "add":
            ...
            if forms_obj.is_valid():
                ...
                models.project_Function.objects.create(**forms_obj.cleaned_data)
            ...
            user_id = request.GET.get('user_id')
            username_log = models.project_UserProfile.objects.get(id=user_id)
            objs = models.project_Function.objects.filter(id=o_id)
            if objs:
                for obj in objs:
                    name = obj.name
                    print('name---->', name)
