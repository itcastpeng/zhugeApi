...
models_quanxian_name = 'project_QuanXian'
models_quanxian_obj = getattr(models, models_quanxian_name)
...
def quanxian(request):
    ...
    if request.method == "GET":
        result_data_list, result_data_tree = get_paixu_data(models_quanxian_obj)
    ...
    if request.method == "POST":
        if oper_type == "add":
            ...
            if forms_obj.is_valid():
                ...
                models_quanxian_obj.objects.create(**forms_obj.cleaned_data)
                response.code = 200
                response.msg = "添加成功"
            else:
                print("验证不通过")
            ...
            # 删除 ID
            objs = models_quanxian_obj.objects.filter(id=o_id)
            if objs:
                obj = objs[0]
                if models_quanxian_obj.objects.filter(pid_id=obj.id):
                    response.code = 304
            ...
            if forms_obj.is_valid():
                ...
                #  查询数据库  用户id
                user_obj = models_quanxian_obj.objects.filter(
                    id=o_id
                )
                #  更新 数据
                if user_obj:
